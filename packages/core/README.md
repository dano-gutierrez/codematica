...
Runtime-safe language modules live under `src/languages/` and `src/language-writing/`. Local language catalogs are indexed from `content/languages/` into schema version 7 of the generated content index; nested phrase character references are validated and the generated file must not be hand-edited. Japanese questionnaire contracts accept single-glyph prompts/labels, while local progress clients retain unique items and synchronize them through the shared 20-item server batch boundary without truncation.
