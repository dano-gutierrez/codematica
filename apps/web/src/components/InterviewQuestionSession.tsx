...
export function InterviewQuestionSession({
  ...
  function advance() {
    const nextStepIndex = Math.min(stepIndex + 1, track.steps.length);
    const status = nextStepIndex >= track.steps.length ? "completed" : "started";

    emitProgress(status, { stepIndex: nextStepIndex, language, trackId: track.id });
  ...
  function restart() {
    ...
    setPreviousTrackId(selectedTrack.id);
    setStepIndex(0);
    emitProgress("started", { stepIndex: 0, language, trackId: selectedTrack.id });
  }

  function emitProgress(status: ProgressStatus, position: Record<string, unknown>) {
    if (onProgressEvent) {
